      ...
      <div className="flex-1 flex items-center justify-center relative z-10 px-12">
        <AnimatePresence mode="wait">
          {isLogin ? (
            ...
            >
              <div className="flex items-center gap-3 mb-10">
                <img src={logo} alt="Funding Pulze" className="h-9 w-9 rounded-lg invert" />
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="relative">
                  <Input
                    ...
                    value={username}
                    onChange={(e) => setUsername(e.target.value)}
                    className="h-12 bg-transparent border-0 border-b-2 border-background/30 rounded-none px-0 pr-8 focus:border-background/60 focus-visible:ring-0 focus-visible:ring-offset-0 placeholder:text-background/50 text-background"
                  />
                  <User size={16} className="absolute right-0 top-1/2 -translate-y-1/2 text-background/50" />
                </div>
                ...
                <div className="relative">
                  <Input
                    ...
                    className="h-12 bg-transparent border-0 border-b-2 border-background/30 rounded-none px-0 pr-8 focus:border-background/60 focus-visible:ring-0 focus-visible:ring-offset-0 placeholder:text-background/50 text-background"
                  />
                  <Mail size={16} className="absolute right-0 top-1/2 -translate-y-1/2 text-background/50" />
                </div>
                ...
                <div className="relative">
                  <Input
                    ...
                    className="h-12 bg-transparent border-0 border-b-2 border-background/30 rounded-none px-0 pr-8 focus:border-background/60 focus-visible:ring-0 focus-visible:ring-offset-0 placeholder:text-background/50 text-background"
                  />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-0 top-1/2 -translate-y-1/2 text-background/50 hover:text-background transition-colors">
                    {showPassword ? <EyeOff size={16} /> : <Eye size={16} />}
                  </button>
                </div>

                <Button type="submit" disabled={loading} className="w-full h-12 rounded-xl font-medium bg-background text-foreground hover:bg-background/90">
                  {loading ? <Spinner dark /> : "Create Account"}
                </Button>

                <div className="flex items-center gap-3 my-1">
                  <div className="flex-1 h-px bg-background/20" />
                  <span className="text-xs text-background/50">or</span>
                  <div className="flex-1 h-px bg-background/20" />
                </div>
                ...
                <button
                  type="button"
                  onClick={handleGoogleSignIn}
                  className="w-full h-12 rounded-xl border border-background/20 bg-background/10 flex items-center justify-center gap-3 text-sm font-medium text-background hover:bg-background/20 transition-colors"
                >
                  <svg width="18" height="18" viewBox="0 0 18 18"><path d="M17.64 9.2c0-.637-.057-1.251-.164-1.84H9v3.481h4.844a4.14 4.14 0 0 1-1.796 2.716v2.259h2.908c1.702-1.567 2.684-3.875 2.684-6.615Z" fill="#4285F4"/><path d="M9 18c2.43 0 4.467-.806 5.956-2.184l-2.908-2.259c-.806.54-1.837.86-3.048.86-2.344 0-4.328-1.584-5.036-3.711H.957v2.332A8.997 8.997 0 0 0 9 18Z" fill="#34A853"/><path d="M3.964 10.706A5.41 5.41 0 0 1 3.682 9c0-.593.102-1.17.282-1.706V4.962H.957A8.997 8.997 0 0 0 0 9c0 1.452.348 2.827.957 4.038l3.007-2.332Z" fill="#FBBC05"/><path d="M9 3.58c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.463.891 11.426 0 9 0A8.997 8.997 0 0 0 .957 4.962L3.964 7.294C4.672 5.166 6.656 3.58 9 3.58Z" fill="#EA4335"/></svg>
                  Continue with Google
                </button>

                <p className="text-sm text-background/60 text-center pt-2">
