...
Deno.serve(async (req) => {
  ...
  try {
    ...
    switch (type) {
      ...
      case 'credentials': {
        const props = { ...baseProps, mt5Login: data.mt5Login, mt5Password: data.mt5Password, mt5Server: data.mt5Server || 'MEXAtlantic-Demo', challengeName: data.challengeName || 'Trading Challenge', accountSize: data.accountSize || '' }
