...
export const DailyDDBreachEmail = ({ siteName = 'Funding Pulze', siteUrl = 'https://fundingpulze.com', recipient, accountNumber, breachValue, limit }: Props) => (
  <Html lang="en" dir="ltr">
    <Head />
    <Preview>⚠️ Daily drawdown limit breached on account #{accountNumber}</Preview>
    <Body style={main}>
      <Container style={container}>
        <Section style={logoSection}><Img src={logoUrl} width="48" height="48" alt="Funding Pulze" style={logoStyle} /></Section>
        <Heading style={h1}>Daily Drawdown Breach ⚠️</Heading>
        <Text style={text}>We regret to inform you that your trading account has been flagged for a daily drawdown breach.</Text>
        <Section style={alertBox}>
          <Text style={alertRow}><strong>Account:</strong> #{accountNumber}</Text>
          <Text style={alertRow}><strong>Daily Drawdown:</strong> {breachValue}</Text>
...
export default DailyDDBreachEmail
